...
class MockRepository:
    ...
    def store_list(self, prefix):
        ...
        ItemInfo = namedtuple('ItemInfo', ['name'])
        return [ItemInfo(name=key.removeprefix(prefix + "/")) 
...
def test_chunked_cache_basic_functionality():
    ...
    result_hash = _write_chunked_chunkindex_to_repo_cache(
        repo, large_data,
        cached_hashes=[],
        force_write=True
    )
    ...
    oversized = [(name, len(data)) for name, data in repo.storage.items() 
    ...
    if manifest['total_size'] != len(large_data):
        print(f"ERROR: Total size mismatch")
        return False
    ...
    if bytes(reassembled_data) == large_data:
        ...
        print(f"ERROR: Reassembled data doesn't match original")
...
def test_error_conditions():
    ...
    result_hash = _write_chunked_chunkindex_to_repo_cache(
        repo, large_data, cached_hashes=[], force_write=True
    )
...
def main():
    """Run all tests"""
    print(f"Chunked ChunkIndex Cache Integration Test")
    ...
    tests = [
        test_chunked_cache_basic_functionality,
        test_integration_with_chunkindex,
